...
class VirFib():
    ...
    def next(self):
        if not self.value:
            ...
            node = VirFib(self.value+self.prev)
...
def is_bst(t):
    ...
    def bst_helper(t, fn):
        if t.is_leaf():
            return t.label
        for b in t.branches:
            return fn(bst_helper(b, fn), t.label)
    if t.is_leaf():
        return True
        ...
        for b in t.branches:
            ...
            if not flag:
                return flag
        if flag and len(t.branches) == 2:
            if t.label < bst_helper(t.branches[0], max) or t.label >= bst_helper(t.branches[1], min):
                flag = False
        return flag
