...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_point = self.place
        start = 0
        while not place_point.is_hive:
            if self.lower_bound <= start < self.upper_bound + int(self.lower_bound == self.upper_bound):
                bee = random_bee(place_point.bees)  # REPLACE THIS LINE
                if bee:
                    return bee
            place_point = place_point.entrance
            start += 1
        return None
        # END Problem 3 and 4
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if isinstance(self.place, ContainerAnt):
            self.place = self.place.place
        super().action(gamestate)
        place_point = self.place.exit
        while place_point:
            if place_point.ant:
                if place_point.ant.is_container:
                    if place_point.ant.ant_contained and not place_point.ant.ant_contained.doubled:
                        place_point.ant.ant_contained.double()
                        place_point.ant.ant_contained.doubled = True
                if not place_point.ant.doubled:
                    place_point.ant.double()
                    place_point.ant.doubled = True
            place_point = place_point.exit
        if isinstance(self.place, ContainerAnt):
            self.place = self.place.place
        # END Problem 12
    ...
    def throw_at(self, target):
        ...
        def func(gamestate):
            if target.slow_turns > 0:
                ...
                Bee.action(target, gamestate)
        if target.slow_turns < self.turns:
            target.slow_turns = self.turns
        target.action = func
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place_point = self.place
        while place_point:
            if place_point.ant and place_point.ant is not self:
                if place_point.ant.is_container:
                    distance_dict[place_point.ant.ant_contained] = distance
                distance_dict[place_point.ant] = distance
            if place_point.bees:
                for bee in place_point.bees:
                    distance_dict[bee] = distance
            distance += 1
            place_point = place_point.entrance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.damage-0.0625*self.insects_shot-0.25*distance
