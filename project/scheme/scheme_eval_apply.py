...
def scheme_eval(expr, env, tail=False):  # Optional third argument is ignored (PROBLEM EC)
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        args = rest.map(lambda sub_rest: scheme_eval(sub_rest, env))
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            val = scheme_eval(expressions.first, env, True) # PROBLEM EC
        else:
