...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(func_name, lambda_procedure)
        return func_name
        # END PROBLEM 10
    else:
...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True) # PROBLEM EC
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True) # PROBLEM EC
...
def do_and_form(expressions, env):
    ...
    return val if is_scheme_true(val) else False
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            return scheme_eval(expressions.first, env, True) # PROBLEM EC
        val = scheme_eval(expressions.first, env)
