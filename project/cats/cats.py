...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return word if score <= limit else typed_word
    # END PROBLEM 5
...
def final_diff(typed, source, limit):
    ...
    return minimum_mewtations(typed, source, FINAL_DIFF_LIMIT)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    score /= len(prompt)
    rdata = {'id': user_id, 'progress': score}
    upload(rdata)
    return score
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        ...
        for j in range(1, len(i)):
            time_consuming.append(i[j] - i[j-1])
        times.append(time_consuming)
...
def fastest_words(match):
    ...
    for word_indice in word_indices:
        fast_player_indice = min(player_indices, key=lambda x: time(match, x, word_indice))
        word = get_word(match, word_indice)
        result[fast_player_indice].append(word)
    return result
    # END PROBLEM 10
