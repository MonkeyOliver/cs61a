...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(0, 100):
        for j in range(0, 100):
            next_roll_num = strategy(i, j)
            if roll_num != next_roll_num:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_scoring = 0
    max_num = 0
    avg = make_averaged(roll_dice, total_samples)
    for i in range(1, 11):
        scoring = avg(i, dice)
...
def tail_strategy(score, opponent_score, threshold=6, num_rolls=6):
...
def square_strategy(score, opponent_score, threshold=6, num_rolls=6):
    ...
    score += tail_score
    if perfect_square(score):  # Implement perfect_square
        score = next_perfect_square(score)  # Implement next_perfect_square
    if score - old_score > threshold:
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
