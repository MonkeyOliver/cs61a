    ...
    def yield_helper(j, k):
        if j == 0:
            ...
            for small_part in yield_helper(j-k, k):
                # print(f"small part is: {small_part}, j is {j}, k is {k}")
                yield [k] + small_part
...
class VendingMachine:
    ...
    def vend(self):
        if not self.stocks:
            ...
            self.stocks -= 1
            if self.funds > 0:
                return_funds = self.funds
                self.funds = 0
                return f'Here is your {self.name} and ${return_funds} change.'
            else:
...
def insert(link, value, index):
    ...
    "*** YOUR CODE HERE ***"
    head_point = link
    while head_point is not Link.empty and index >= 0:
        if index == 0:
            node = Link(head_point.first, head_point.rest)
            head_point.first = value
            head_point.rest = node
            return
        index -= 1
        head_point = head_point.rest
    if index >= 0:
...
def long_paths(t, n):
    ...
    "*** YOUR CODE HERE ***"
    if t.is_leaf():
        return [[t.label]] if n <= 0 else []
    else:
        new_paths = []
        for b in t.branches:
            paths = long_paths(b, n - 1)
            for path in paths:
                new_paths.append([t.label] + path)
        return new_paths
